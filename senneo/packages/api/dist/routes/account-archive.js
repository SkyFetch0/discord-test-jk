...
exports.accountArchiveRouter = accountArchiveRouter;
...
// ── Discord REST helper ─────────────────────────────────────────────────────
function discordGet(token, endpoint) {
    return (0, discord_proxy_1.discordApiGet)(endpoint, { token, timeoutMs: 10_000 }).catch(() => null);
}
// ── Name cache helper ───────────────────────────────────────────────────────
...
// ── Router ──────────────────────────────────────────────────────────────────
function accountArchiveRouter(db) {
    ...
    router.delete('/failed/:accountId', async (req, res) => {
        try {
    ...
    router.post('/accounts/:accountId', async (req, res) => {
        try {
            ...
            const existing = await db.execute(`SELECT account_id FROM ${KEYSPACE}.archived_accounts WHERE account_id = ?`, [accountId], P);
            ...
            let username = '', avatar = '';
            const accInfo = await db.execute(`SELECT username, avatar FROM ${KEYSPACE}.account_info WHERE account_id = ?`, [accountId], P).catch(() => null);
            if (accInfo && accInfo.rowLength > 0) {
                username = accInfo.rows[0]['username'] ?? '';
                avatar = accInfo.rows[0]['avatar'] ?? '';
            }
            // 2. Snapshot guilds from account_guilds
            const guildsResult = await db.execute(`SELECT guild_id, guild_name, guild_icon, guild_owner FROM ${KEYSPACE}.account_guilds WHERE account_id = ?`, [accountId], P);
            // 3. Get invite codes for these guilds from invite_pool
            const guildIds = guildsResult.rows.map(r => r['guild_id']);
            ...
            if (guildIds.length > 0) {
                ...
                for (const row of poolResult.rows) {
                    ...
                    if (gid && code && guildIds.includes(gid))
            ...
            const poolAssigned = await db.execute(`SELECT guild_id, guild_name, guild_icon, invite_code, status, owner_account_id, assigned_account_id, owner_account_name, assigned_account_name FROM ${KEYSPACE}.invite_pool`, [], P);
            const memberGuildIdSet = new Set(guildIds);
            const assignedGuilds = [];
            for (const row of poolAssigned.rows) {
                const ownerAccId = row['owner_account_id'] ?? '';
                const assignedAccId = row['assigned_account_id'] ?? '';
                const ownerName = row['owner_account_name'] ?? '';
                const assignedName = row['assigned_account_name'] ?? '';
                const gid = row['guild_id'];
                if (!gid)
                    continue;
            ...
            for (const row of guildsResult.rows) {
                const gid = row['guild_id'];
                await db.execute(`INSERT INTO ${KEYSPACE}.archived_account_guilds (account_id, guild_id, guild_name, guild_icon, invite_code, membership) VALUES (?,?,?,?,?,?)`, [accountId, gid, row['guild_name'] ?? '', row['guild_icon'] ?? '', inviteMap.get(gid) ?? '', 'member'], P);
            }
            // Also write assigned/to_join guilds
            for (const ag of assignedGuilds) {
                await db.execute(`INSERT INTO ${KEYSPACE}.archived_account_guilds (account_id, guild_id, guild_name, guild_icon, invite_code, membership) VALUES (?,?,?,?,?,?)`, [accountId, ag.guildId, ag.guildName, ag.guildIcon, ag.inviteCode, ag.membership], P);
            }
            // 6. Snapshot scrape targets + checkpoints for this account
            const targetsResult = await db.execute(`SELECT channel_id, guild_id, account_id, pinned_account_id FROM ${KEYSPACE}.scrape_targets`, [], P);
            const accountTargets = targetsResult.rows.filter(r => (r['pinned_account_id'] ?? r['account_id']) === accountId);
            // Get channel names
            const channelIds = accountTargets.map(r => r['channel_id']);
            ...
            for (const row of accountTargets) {
                ...
                const cpResult = await db.execute(`SELECT total_scraped, complete, cursor_id, newest_message_id FROM ${KEYSPACE}.scrape_checkpoints WHERE channel_id = ?`, [cid], P).catch(() => null);
                const cp = cpResult && cpResult.rowLength > 0 ? cpResult.rows[0] : null;
                const ts = cp ? BigInt(Number(cp['total_scraped'] ?? 0)) : BigInt(0);
                totalScrapedAll += ts;
                await db.execute(`INSERT INTO ${KEYSPACE}.archived_account_channels (account_id, channel_id, guild_id, channel_name, total_scraped, complete, cursor_id, newest_message_id) VALUES (?,?,?,?,?,?,?,?)`, [accountId, cid, gid, nameMap.get(cid) ?? '', Number(ts), cp?.['complete'] ?? false, cp?.['cursor_id'] ?? null, cp?.['newest_message_id'] ?? null], P);
            }
            const totalGuildCount = guildsResult.rowLength + assignedGuilds.length;
            // 7. Write archived_accounts header
            await db.execute(`INSERT INTO ${KEYSPACE}.archived_accounts (account_id, username, avatar, archived_at, reason, guild_count, channel_count, total_scraped) VALUES (?,?,?,?,?,?,?,?)`, [accountId, username, avatar, new Date(), reason, totalGuildCount, channelIds.length, Number(totalScrapedAll)], P);
    ...
    router.get('/accounts/:accountId', async (req, res) => {
        try {
            ...
            const guilds = guildsResult.rows.map(g => ({
                ...
                guildIcon: g['guild_icon'] ?? null,
                inviteCode: g['invite_code'] ?? null,
                membership: g['membership'] ?? 'member',
            ...
            const channels = channelsResult.rows.map(c => ({
                ...
                cursorId: c['cursor_id'] ?? null,
                newestMessageId: c['newest_message_id'] ?? null,
    ...
    router.post('/accounts/:accountId/transfer', async (req, res) => {
        try {
            const { accountId } = req.params;
            ...
            const headerResult = await db.execute(`SELECT * FROM ${KEYSPACE}.archived_accounts WHERE account_id = ?`, [accountId], P);
            if (headerResult.rowLength === 0) {
                return res.status(404).json({ error: 'Arsivlenmis hesap bulunamadi' });
            }
            ...
            const newAccountId = userInfo.id;
            const newUsername = userInfo.username ?? '';
            let newAccountIdx = -1;
            if (newAccountId !== accountId) {
                const existingTargetAccount = await db.execute(`SELECT account_id FROM ${KEYSPACE}.account_info WHERE account_id = ?`, [newAccountId], P).catch(() => null);
            ...
            const previousAccountInfo = await db.execute(`SELECT email, account_password, mail_password, mail_site FROM ${KEYSPACE}.account_info WHERE account_id = ?`, [accountId], P).catch(() => null);
            const previousEmail = previousAccountInfo?.rows[0]?.['email'] ?? null;
            const previousAccountPassword = previousAccountInfo?.rows[0]?.['account_password'] ?? null;
            const previousMailPassword = previousAccountInfo?.rows[0]?.['mail_password'] ?? null;
            const previousMailSite = previousAccountInfo?.rows[0]?.['mail_site'] ?? null;
            // Save new account to account_info
            await db.execute(`INSERT INTO ${KEYSPACE}.account_info (account_id, discord_id, username, avatar, last_fetched, email, account_password, mail_password, mail_site) VALUES (?,?,?,?,?,?,?,?,?)`, [newAccountId, newAccountId, newUsername, userInfo.avatar ?? '', new Date(), previousEmail, previousAccountPassword, previousMailPassword, previousMailSite], P).catch(() => { });
            await db.execute(`INSERT INTO ${KEYSPACE}.token_account_map (token_key, account_id, username, updated_at) VALUES (?,?,?,?)`, [token.slice(-16), newAccountId, newUsername, new Date()], P).catch(() => { });
            const newTokenKey = token.slice(-16);
            const tokenMapResult = await db.execute(`SELECT token_key, account_id FROM ${KEYSPACE}.token_account_map`, [], P).catch(() => null);
            const staleTokenKeys = new Set((tokenMapResult?.rows ?? [])
                .filter(row => (row['account_id'] ?? '') === accountId)
                .map(row => row['token_key'] ?? '')
                .filter(tokenKey => !!tokenKey && tokenKey !== newTokenKey));
            try {
                const raw = fs_1.default.existsSync(ACCOUNTS_FILE) ? JSON.parse(fs_1.default.readFileSync(ACCOUNTS_FILE, 'utf-8')) : {};
                const storedAccounts = Array.isArray(raw.accounts) ? raw.accounts : [];
                const nextAccounts = storedAccounts.filter((entry) => typeof entry?.token === 'string' && !staleTokenKeys.has(entry.token.slice(-16)));
                ...
                fs_1.default.writeFileSync(ACCOUNTS_FILE, JSON.stringify({ accounts: nextAccounts }, null, 2));
            ...
            if (staleTokenKeys.size > 0) {
                await Promise.all([...staleTokenKeys].map(tokenKey => db.execute(`DELETE FROM ${KEYSPACE}.token_account_map WHERE token_key = ?`, [tokenKey], P).catch(() => { })));
            }
            // Get archived guilds
            const guildsResult = await db.execute(`SELECT * FROM ${KEYSPACE}.archived_account_guilds WHERE account_id = ?`, [accountId], P);
            ...
            for (const row of guildsResult.rows) {
                ...
                const existingInvite = await db.execute(`SELECT invite_code FROM ${KEYSPACE}.invite_pool WHERE invite_code = ?`, [inviteCode], P).catch(() => null);
                if (!existingInvite || existingInvite.rowLength === 0) {
                    await db.execute(`INSERT INTO ${KEYSPACE}.invite_pool (invite_code, guild_id, guild_name, guild_icon, status, owner_account_id, owner_account_name, assigned_account_id, assigned_account_name, created_at, checked_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [inviteCode, guildId, guildName, guildIcon, status, ownerField, ownerName, assignedField, assignedName, now, now], P);
                    invitesCreated++;
                }
                else {
                    // Update existing entry
                    await db.execute(`UPDATE ${KEYSPACE}.invite_pool SET owner_account_id = ?, owner_account_name = ?, assigned_account_id = ?, assigned_account_name = ?, status = ?, checked_at = ? WHERE invite_code = ?`, [ownerField, ownerName, assignedField, assignedName, status, now, inviteCode], P);
                }
            }
            // Re-create scrape targets for channels that were being scraped
            const channelsResult = await db.execute(`SELECT * FROM ${KEYSPACE}.archived_account_channels WHERE account_id = ?`, [accountId], P);
            ...
            for (const row of channelsResult.rows) {
                ...
                if (!isSameAccount) {
                    await db.execute(`DELETE FROM ${KEYSPACE}.account_targets_by_account WHERE account_id = ? AND channel_id = ?`, [accountId, channelId], P).catch(() => { });
                }
                // Only re-create targets for incomplete channels
                if (!complete) {
                    await db.execute(`INSERT INTO ${KEYSPACE}.scrape_targets (channel_id, guild_id, account_id, account_idx, pinned_account_id, pinned_account_idx, created_at) VALUES (?,?,?,?,?,?,?)`, [channelId, guildId, newAccountId, newAccountIdx >= 0 ? newAccountIdx : null, newAccountId, newAccountIdx >= 0 ? newAccountIdx : null, now], P).catch(() => { }); // Ignore if already exists
                    await db.execute(`INSERT INTO ${KEYSPACE}.account_targets_by_account (account_id, channel_id, guild_id, label, account_idx, active_account_id, active_account_idx, pinned_account_id, pinned_account_idx, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`, [newAccountId, channelId, guildId, channelName, newAccountIdx >= 0 ? newAccountIdx : null, newAccountId, newAccountIdx >= 0 ? newAccountIdx : null, newAccountId, newAccountIdx >= 0 ? newAccountIdx : null, now], P).catch(() => { });
                    targetsCreated++;
                }
            }
            if (!isSameAccount) {
            ...
            await db.execute(`UPDATE ${KEYSPACE}.archived_accounts SET transferred_to = ?, transferred_at = ? WHERE account_id = ?`, [newAccountId, now, accountId], P);
...
//# sourceMappingURL=account-archive.js.map