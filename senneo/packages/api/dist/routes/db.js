...
let _dedupError = null;
let _dedupResult = null;
 ...
 */
async function enrichUserRowsFromMessages(ch, rows) {
    if (rows.length === 0)
        return rows;
      ...
      `,
        ...
        return rows.map(row => {
            ...
            if (!m)
                return row;
            const out = { ...row };
            if (m._msg_count != null)
                out.msg_count = m._msg_count;
            if (m._first_seen != null)
                out.first_seen = m._first_seen;
            if (m._last_seen != null)
                out.last_seen = m._last_seen;
            const av = String(row.author_avatar ?? '').trim();
            const isValidAv = av && av !== '0' && av !== 'null' && av !== 'undefined';
            const best = String(m._best_avatar ?? '').trim();
            if (!isValidAv && best && best !== '0')
                out.author_avatar = best;
            ...
            if (aggMask > 0)
                out.badge_mask = String(existingMask | aggMask);
            return out;
        });
    }
    catch {
...
function dbRouter(scylla, ch) {
    const router = (0, express_1.Router)();
    //  ClickHouse: list tables 
    router.get('/ch/tables', async (_req, res) => {
        try {
            const r = await ch.query({
    ...
    router.get('/ch/tables/:table/rows', async (req, res) => {
    ...
    //  ClickHouse: run SQL 
    router.post('/ch/query', async (req, res) => {
        const { sql } = req.body;
        if (!sql?.trim())
    ...
    router.get('/ch/analytics/topusers', async (req, res) => {
    ...
    router.get('/ch/analytics/topchannels', async (req, res) => {
    ...
    router.get('/ch/analytics/activity', async (req, res) => {
    ...
    router.get('/ch/analytics/hourly', async (_req, res) => {
        try {
            const r = await ch.query({
    ...
    router.get('/ch/analytics/search', async (req, res) => {
    ...
    router.get('/ch/analytics/content-types', async (req, res) => {
        const days = Math.min(parseInt(req.query['days'] ?? '30'), 90);
        try {
            const r = await ch.query({
                query: `SELECT
    ...
    router.get('/ch/analytics/media-types', async (req, res) => {
        const days = Math.min(parseInt(req.query['days'] ?? '30'), 90);
        try {
            const r = await ch.query({
    ...
    router.get('/ch/analytics/msg-size', async (req, res) => {
        const days = Math.min(parseInt(req.query['days'] ?? '30'), 90);
        try {
            const r = await ch.query({
                query: `SELECT
    ...
    router.get('/ch/analytics/user', async (req, res) => {
        const authorId = (req.query['authorId'] ?? '').trim();
        const name = (req.query['name'] ?? '').trim();
        const limit = Math.min(parseInt(req.query['limit'] ?? '20'), 100);
        if (!authorId && !name)
            return res.status(400).json({ error: 'authorId or name is required' });
        try {
            if (authorId && /^\d{17,20}$/.test(authorId)) {
                ...
                const [countsRaw] = await countR.json();
                let user = rows[0] ?? null;
                if (user && countsRaw) {
                    const { best_avatar_from_msgs, badge_mask_agg, ...counts } = countsRaw;
    ...
    router.get('/ch/analytics/user-history', async (req, res) => {
        const authorId = (req.query['authorId'] ?? '').trim();
        if (!authorId || !/^\d{17,20}$/.test(authorId))
    ...
    router.get('/ch/analytics/heatmap', async (req, res) => {
        const days = Math.min(parseInt(req.query['days'] ?? '30'), 90);
        try {
            const r = await ch.query({
    ...
    router.get('/ch/analytics/weekly-growth', async (req, res) => {
    ...
    router.get('/ch/analytics/channel-hourly', async (req, res) => {
    ...
    router.get('/ch/analytics/overview', async (req, res) => {
        const days = Math.min(parseInt(req.query['days'] ?? '30'), 90);
        try {
            ...
            const [t] = await totals.json();
            const d = await daily.json();
            const h = await hourly.json();
            const [ct] = await contentTypes.json();
    ...
    router.get('/ch/dedup/status', async (_req, res) => {
        try {
            const r = await ch.query({
                query: `
          SELECT
    ...
    router.post('/ch/dedup/run', async (_req, res) => {
        ...
        (async () => {
            ...
            try {
                ...
                const partRows = await partResult.json();
                for (const { partition } of partRows) {
                    ...
                    }).catch((e) => console.warn(`[dedup] partition ${partition} skip:`, e?.message?.slice(0, 80)));
                }
    ...
    router.get('/scylla/tables', async (_req, res) => {
        try {
            const r = await scylla.execute(`SELECT table_name FROM system_schema.tables WHERE keyspace_name = ?`, [KEYSPACE], { prepare: true });
    ...
    router.post('/scylla/query', async (req, res) => {
        const { cql } = req.body;
        if (!cql?.trim())
            return res.status(400).json({ error: 'cql is required' });
