...
async function writeErrors(ch, entries) {
    if (entries.length === 0)
        return;
...
function errorsRouter(ch) {
    ...
    router.get('/', async (req, res) => {
        ...
        const accountIdx = req.query['accountIdx'];
        const since = req.query['since']; // ISO or "1h", "24h", "7d"
        ...
        if (channelId) {
            conditions.push(`channel_id = {chId:String}`);
            params.chId = channelId;
        }
        if (guildId) {
            conditions.push(`guild_id = {gId:String}`);
            params.gId = guildId;
        }
        if (accountId) {
            conditions.push(`account_id = {aId:String}`);
            params.aId = accountId;
        }
        if (accountIdx) {
            conditions.push(`account_idx = {aIdx:Int32}`);
            params.aIdx = parseInt(accountIdx, 10);
        }
        if (q) {
