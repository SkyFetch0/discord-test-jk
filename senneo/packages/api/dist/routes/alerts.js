...
exports.evaluateMessages = evaluateMessages;
...
async function initSchema(scylla) {
...
const _evaluated = new Map();
const MAX_EVAL_HISTORY = 1000;
function alreadyEvaluated(ruleId, messageId) {
    return _evaluated.get(ruleId)?.has(messageId) ?? false;
}
function markEvaluated(ruleId, messageId) {
    if (!_evaluated.has(ruleId))
        _evaluated.set(ruleId, new Set());
...
function matchesPattern(content, pattern, mode) {
    if (!content || !pattern)
        return false;
    if (mode === 'substring') {
 ...
 */
async function evaluateMessages(scylla, messages) {
    if (!_rulesLoaded)
    ...
    for (const msg of messages) {
        for (const rule of activeRules) {
            if (alreadyEvaluated(rule.id, msg.message_id))
                continue;
            if (rule.channelIds.length > 0 && !rule.channelIds.includes(msg.channel_id))
                continue;
            if (!matchesPattern(msg.content, rule.pattern, rule.matchMode))
                continue;
            ...
            scylla.execute(`UPDATE ${KEYSPACE}.alert_rules SET last_triggered_at = ?, last_triggered_msg_id = ?, trigger_count = ? WHERE id = ?`, [new Date(), msg.message_id, rule.triggerCount + 1, rule.id]).catch(() => { });
            rule.triggerCount++;
...
function alertsRouter(scylla) {
    ...
    router.post('/', async (req, res) => {
        const { pattern, matchMode, channelIds, webhookUrl } = req.body;
        if (!pattern?.trim())
        ...
        await scylla.execute(`INSERT INTO ${KEYSPACE}.alert_rules (id, pattern, match_mode, channel_ids, enabled, webhook_url, created_at, trigger_count) VALUES (?,?,?,?,?,?,?,?)`, [id, pattern.trim(), mode, channels, true, webhookUrl.trim(), new Date(), 0]);
        await refreshCache(scylla);
    ...
    router.put('/:id', async (req, res) => {
        const { id } = req.params;
        const { pattern, matchMode, channelIds, enabled, webhookUrl } = req.body;
        ...
        if (matchMode != null) {
            ...
            params.push(matchMode === 'whole' ? 'whole' : 'substring');
        }
        if (channelIds != null) {
            updates.push('channel_ids = ?');
            params.push(Array.isArray(channelIds) ? channelIds : []);
        }
        if (enabled != null) {
    ...
    router.post('/test', async (req, res) => {
        const { pattern, matchMode } = req.body;
        if (!pattern?.trim())
            return res.status(400).json({ error: 'pattern is required' });
        const mode = matchMode === 'whole' ? 'whole' : 'substring';
