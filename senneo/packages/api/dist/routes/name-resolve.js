...
exports.fetchNamesByIds = fetchNamesByIds;
exports.fetchIconsByIds = fetchIconsByIds;
exports.enrichMessagesWithNames = enrichMessagesWithNames;
 ...
 */
async function fetchNamesByIds(scylla, ids) {
    const map = new Map();
 ...
 */
async function fetchIconsByIds(scylla, ids) {
    const map = new Map();
    const uniq = [...new Set(ids)].filter(Boolean);
    for (let i = 0; i < uniq.length; i += NAME_IN_CHUNK) {
        const slice = uniq.slice(i, i + NAME_IN_CHUNK);
        ...
        for (const row of result.rows) {
            const icon = row['icon'];
            if (icon && icon !== '0' && icon !== 'null')
                map.set(row['id'], icon);
        }
    }
    return map;
}
async function fetchUserProfiles(ch, authorIds) {
    const map = new Map();
                ...
                for (const row of rows) {
                    ...
                    if (bestAvatar && bestAvatar !== '0') {
                        ...
                        if (existing) {
                            ...
                            if (!existing.display_name && row.best_display_name)
                                existing.display_name = String(row.best_display_name);
                        }
                        else {
 ...
 */
async function enrichMessagesWithNames(scylla, rows, ch) {
    if (rows.length === 0)
        return rows;
    ...
    let profiles = null;
    if (ch) {
        const needsBackfill = [...new Set(rows
    ...
    for (const row of rows) {
        ...
        if (profiles && (!r['author_avatar'] || String(r['author_avatar']) === '' || String(r['author_avatar']) === '0')) {
            ...
            if (profile) {
                if (profile.author_avatar)
                    r['author_avatar'] = profile.author_avatar;
                if (profile.display_name)
                    r['display_name'] = profile.display_name;
...
//# sourceMappingURL=name-resolve.js.map