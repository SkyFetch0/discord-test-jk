...
async function getTokenForAccount(db, accountId) {
...
function discordApiGet(endpoint, token) {
    return (0, discord_proxy_1.discordApiGet)(endpoint, { token, timeoutMs: 10_000 });
}
async function verifyGuildMembership(token, guildId) {
    try {
...
async function validateChannelsForGuild(token, guildId, channelIds) {
    const validated = [];
    for (const channelId of channelIds) {
        let channel;
        try {
            channel = await discordApiGet(`/channels/${channelId}`, token);
        }
        catch {
...
function setTokenCookie(res, token) {
    ...
    res.setHeader('Set-Cookie', `${COOKIE_NAME}=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${maxAge}${secureFlag}`);
}
...
setInterval(() => {
    const now = Date.now();
    for (const [ip, exp] of BLOCKED_UNTIL)
        if (now > exp + LOGIN_BLOCK_MS)
            BLOCKED_UNTIL.delete(ip);
    for (const [ip, e] of LOGIN_ATTEMPTS)
        if (now - e.windowStart > LOGIN_WINDOW_MS * 2)
            LOGIN_ATTEMPTS.delete(ip);
}, 10 * 60_000).unref();
// ── Schema init + seed ───────────────────────────────────────────────────
async function initAuthSchema(db) {
    await db.execute(`
  ...
  `).catch(() => { });
    ...
    const existing = await db.execute(`SELECT username FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, ['Nimdes']);
    if (existing.rowLength === 0) {
        ...
        await db.execute(`INSERT INTO ${KEYSPACE}.dashboard_users (username, password_hash, display_name, role, created_at, created_by)
       VALUES (?, ?, ?, ?, ?, ?)`, ['Nimdes', hash, 'Nimdes', 'admin', new Date(), 'system']);
        if (generated) {
            console.log('╔══════════════════════════════════════════════════════╗');
...
async function logActivity(db, username, action, detail, ip) {
    db.execute(`INSERT INTO ${KEYSPACE}.user_activity_log (username, ts, action, detail, ip) VALUES (?,?,?,?,?)`, [username, new Date(), action, detail, ip]).catch(() => { });
}
async function createSession(db, username, ip, ua) {
    const sid = genId();
    const now = new Date();
    await db.execute(`INSERT INTO ${KEYSPACE}.user_sessions (session_id, username, created_at, last_active, ip, user_agent, revoked) VALUES (?,?,?,?,?,?,?)`, [sid, username, now, now, ip, ua, false]);
    return sid;
}
async function touchLastSeen(db, username) {
    db.execute(`UPDATE ${KEYSPACE}.user_last_seen SET last_seen = ?, status = 'online' WHERE username = ?`, [new Date(), username]).catch(() => { });
}
// ── Router ───────────────────────────────────────────────────────────────
function authRouter(db) {
    ...
    router.post('/login', async (req, res) => {
        try {
            ...
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, [username]);
            if (result.rowLength === 0) {
                ...
                await bcryptjs_1.default.compare(password, '$2a$12$invalidhashpaddingtomakeconstanttime000000000000000000000');
            ...
            try {
                ...
                if (policyRes.rowLength > 0) {
                    ...
                    if (pol['enforce'] === true && Number(pol['max_days'] ?? 0) > 0) {
                        ...
                        const refMs = changedAt ? changedAt.getTime() : row['created_at']?.getTime() ?? 0;
            ...
            if (user.role !== 'admin') {
                try {
                    ...
                    if (permsRes.rowLength > 0)
                        allowedPages = permsRes.rows.map(r => r['page_id']);
                }
                catch { /* non-fatal */ }
            }
            const token = jsonwebtoken_1.default.sign({ ...user, sid: sessionId }, JWT_SECRET, { expiresIn: JWT_EXPIRY });
            ...
            if (policyRes.rowLength > 0) {
                ...
                if (pol['enforce'] === true && Number(pol['max_days'] ?? 0) > 0) {
                    ...
                    if (userRes.rowLength > 0) {
                        ...
                        const changedAt = ur['password_changed_at'];
                        const cutoffMs = Number(pol['max_days']) * 86_400_000;
                        const refMs = changedAt ? changedAt.getTime() : ur['created_at']?.getTime() ?? 0;
                        if (Date.now() - refMs > cutoffMs)
    ...
    router.get('/users', requireAdmin, async (_req, res) => {
        try {
            const result = await db.execute(`SELECT username, display_name, role, created_at, created_by FROM ${KEYSPACE}.dashboard_users`);
    ...
    router.post('/users', requireAdmin, async (req, res) => {
        try {
            ...
            const existing = await db.execute(`SELECT username FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, [username]);
            if (existing.rowLength > 0) {
            ...
            await db.execute(`INSERT INTO ${KEYSPACE}.dashboard_users (username, password_hash, display_name, role, created_at, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`, [username, hash, displayName ?? username, validRole, new Date(), admin.username]);
    ...
    router.delete('/users/:username', requireAdmin, async (req, res) => {
        try {
            ...
            // Delete user
            await db.execute(`DELETE FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, [target]);
    ...
    router.put('/users/:username', requireAdmin, async (req, res) => {
        try {
            ...
            // Check exists
            const existing = await db.execute(`SELECT username FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, [target]);
            if (existing.rowLength === 0) {
            ...
            if (password) {
                const hash = await bcryptjs_1.default.hash(password, 12);
                await db.execute(`UPDATE ${KEYSPACE}.dashboard_users SET password_hash = ? WHERE username = ?`, [hash, target]);
            }
            if (displayName) {
                await db.execute(`UPDATE ${KEYSPACE}.dashboard_users SET display_name = ? WHERE username = ?`, [displayName, target]);
            }
            if (role && (role === 'admin' || role === 'user')) {
                await db.execute(`UPDATE ${KEYSPACE}.dashboard_users SET role = ? WHERE username = ?`, [role, target]);
            }
            res.json({ ok: true });
        }
        catch (err) {
    ...
    router.post('/tasks', requireAdmin, async (req, res) => {
        try {
            ...
            if (!assignedTo || !title) {
                res.status(400).json({ error: 'assignedTo ve title gerekli' });
                return;
            }
            const admin = req.user;
            const taskId = genId();
            ...
            await db.execute(`INSERT INTO ${KEYSPACE}.user_tasks (task_id, assigned_to, title, description, status, priority, task_type, deadline, created_by, created_at, updated_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [taskId, assignedTo, title, description ?? '', 'pending', priority ?? 'medium', 'generic', dl, admin.username, now, now]);
            const notifId = genId();
            await db.execute(`INSERT INTO ${KEYSPACE}.user_notifications (id, username, title, message, type, read, created_at) VALUES (?,?,?,?,?,?,?)`, [notifId, assignedTo, 'Yeni Gorev', `"${title}" gorevi size atandi`, 'task', false, now]).catch(() => { });
            const ip = getClientIp(req);
    ...
    router.post('/distribute-tasks', requireAdmin, async (req, res) => {
        try {
            const admin = req.user;
            const now = new Date();
            // 1. Get guilds that need joining (to_join in invite_pool) — source of truth
            const poolResult = await db.execute(`SELECT invite_code, guild_id, guild_name, guild_icon, status, assigned_account_id, assigned_account_name FROM ${KEYSPACE}.invite_pool`);
            const toJoin = poolResult.rows.filter(r => r['status'] === 'to_join' && r['guild_id'] && r['assigned_account_id']);
            if (toJoin.length === 0) {
                res.json({ ok: true, distributed: 0, message: 'Katilim bekleyen sunucu yok' });
                return;
            }
            ...
            const poolAssignMap = new Map(); // guild_id → current assigned_account_id
            for (const g of toJoin) {
                const gid = g['guild_id'];
                const accId = g['assigned_account_id'] ?? '';
                if (gid && accId)
                    poolAssignMap.set(gid, accId);
            }
            ...
            for (const row of existingTasks.rows) {
                ...
                if (!gid || st === 'completed')
                    continue;
                const taskAccId = row['account_id'] ?? '';
                ...
                if (currentPoolAccId && taskAccId && currentPoolAccId !== taskAccId) {
                    await db.execute(`DELETE FROM ${KEYSPACE}.user_tasks WHERE assigned_to = ? AND task_id = ?`, [row['assigned_to'], row['task_id']]).catch(() => { });
                    staleCleaned++;
                }
                else {
            ...
            if (staleCleaned > 0)
                console.log(`[distribute] Cleaned ${staleCleaned} stale tasks (account reassigned)`);
            const newGuilds = toJoin.filter(r => !existingGuildIds.has(r['guild_id']));
            if (newGuilds.length === 0) {
                res.json({ ok: true, distributed: 0, message: 'Tum sunucular zaten gorev olarak atanmis' });
                return;
            }
            // 4. Get non-admin users
            const usersResult = await db.execute(`SELECT username, display_name, role FROM ${KEYSPACE}.dashboard_users`);
            const workers = usersResult.rows
                .filter(r => r['role'] !== 'admin')
                .map(r => r['username']);
            if (workers.length === 0) {
                res.status(400).json({ error: 'Gorev atanacak kullanici yok (admin olmayan)' });
                return;
            }
            // 5. Group guilds by assigned_account_id (Discord account)
            const byAccount = new Map();
            for (const g of newGuilds) {
                const accId = g['assigned_account_id'] ?? 'unknown';
                const accName = g['assigned_account_name'] ?? accId;
            ...
            for (let gi = 0; gi < accountGroups.length; gi++) {
                ...
                for (const guild of group.guilds) {
                    ...
                    const guildName = guild['guild_name'] ?? guildId;
                    ...
                    const cleanName = group.accountName.includes(' - ')
                        ...
                        : group.accountName;
                    await db.execute(`INSERT INTO ${KEYSPACE}.user_tasks (task_id, assigned_to, title, description, status, priority, task_type, invite_code, guild_id, guild_name, account_id, account_name, created_by, created_at, updated_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [taskId, assignTo, `${guildName} sunucusuna katil`,
                ...
                const cleanName = group.accountName.includes(' - ')
                    ? group.accountName.split(' - ')[0].trim()
                    : group.accountName;
                db.execute(`INSERT INTO ${KEYSPACE}.user_notifications (id, username, title, message, type, read, created_at) VALUES (?,?,?,?,?,?,?)`, [nid, assignTo, 'Yeni Gorevler', `${cleanName} hesabi icin ${group.guilds.length} sunucu gorevi atandi`, 'task', false, now]).catch(() => { });
            }
            const ip = getClientIp(req);
    ...
    router.put('/tasks/:taskId', requireAuth, async (req, res) => {
        try {
            ...
            if (channelIds && Array.isArray(channelIds) && guildId) {
                const realGuildId = guildId;
                const validIds = [...new Set(channelIds.filter(id => typeof id === 'string' && /^\d{17,20}$/.test(id)))];
                if (validIds.length === 0) {
                    res.status(400).json({ error: 'Gecerli kanal ID bulunamadi' });
                    return;
                }
                const now = new Date();
                // 1. Look up task data to get account info
                const taskResult = await db.execute(`SELECT * FROM ${KEYSPACE}.user_tasks WHERE assigned_to = ? AND task_id = ?`, [owner, taskId]);
                if (taskResult.rowLength === 0) {
                    res.status(404).json({ error: 'Gorev bulunamadi' });
                    return;
                }
                const task = taskResult.rows[0];
                const accountId = task['account_id'] ?? null;
                const accountName = task['account_name'] ?? '';
                const inviteCode = task['invite_code'] ?? null;
                const guildName = task['guild_name'] ?? '';
                // 2. Get account's token for verification
                if (!accountId) {
                    res.status(400).json({ error: 'Gorevde hesap bilgisi eksik' });
                    return;
                }
                const token = await getTokenForAccount(db, accountId);
                if (!token) {
                    res.status(400).json({ error: 'Hesap tokeni bulunamadi — hesap sistemde aktif degil' });
                    return;
                }
                ...
                if (!isMember) {
                    ...
                    const nid = genId();
                    db.execute(`INSERT INTO ${KEYSPACE}.user_notifications (id, username, title, message, type, read, created_at) VALUES (?,?,?,?,?,?,?)`, [nid, owner, 'Dogrulama Basarisiz', `${guildName || realGuildId} — Hesap sunucuya katilmamis. Davet linkiyle katildigindan ve dogru kanal ID girdiginizden emin olun.`, 'warning', false, now]).catch(() => { });
                ...
                for (const chId of validIds) {
                    const existing = await db.execute(`SELECT channel_id FROM ${KEYSPACE}.scrape_targets WHERE channel_id = ?`, [chId]).catch(() => null);
                    if (existing && existing.rowLength > 0)
                        duplicates.push(chId);
                }
                ...
                for (const channel of validatedChannels.channels) {
                    await db.execute(`INSERT INTO ${KEYSPACE}.scrape_targets (channel_id, guild_id, account_id, account_idx, pinned_account_id, pinned_account_idx, created_at) VALUES (?,?,?,?,?,?,?)`, [channel.id, realGuildId, accountId, accountIdx >= 0 ? accountIdx : null, accountId, accountIdx >= 0 ? accountIdx : null, now]).catch(() => { });
                    await db.execute(`INSERT INTO ${KEYSPACE}.account_targets_by_account (account_id, channel_id, guild_id, label, account_idx, active_account_id, active_account_idx, pinned_account_id, pinned_account_idx, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`, [accountId, channel.id, realGuildId, channel.name, accountIdx >= 0 ? accountIdx : null, accountId, accountIdx >= 0 ? accountIdx : null, accountId, accountIdx >= 0 ? accountIdx : null, now]).catch(() => { });
                    if (channel.name) {
                        await db.execute(`INSERT INTO ${KEYSPACE}.name_cache (id, name, kind) VALUES (?,?,?)`, [channel.id, channel.name, 'channel']).catch(() => { });
                    }
                }
                if (guildName) {
                    await db.execute(`INSERT INTO ${KEYSPACE}.name_cache (id, name, kind) VALUES (?,?,?)`, [realGuildId, guildName, 'guild']).catch(() => { });
                }
                ...
                if (!resolvedCode) {
                    ...
                    for (const prow of poolScan.rows) {
                        if (prow['guild_id'] === realGuildId) {
                            resolvedCode = prow['invite_code'];
                            break;
                        }
                    }
                }
                if (resolvedCode) {
                    await db.execute(`UPDATE ${KEYSPACE}.invite_pool SET status = 'already_in', owner_account_id = ?, owner_account_name = ?, assigned_account_id = ?, assigned_account_name = ?, checked_at = ? WHERE invite_code = ?`, [accountId, accountName, null, null, now, resolvedCode]).catch(e => console.warn('[task] invite_pool update failed:', e?.message));
                }
                // 7. Update guild membership tables (critical for membership display)
                try {
                    await db.execute(`INSERT INTO ${KEYSPACE}.guild_accounts (guild_id, account_id, guild_name, last_synced) VALUES (?,?,?,?)`, [realGuildId, accountId, guildName, now]);
                }
                catch (e) {
                    console.warn('[task] guild_accounts write failed:', e?.message);
                }
                try {
                    await db.execute(`INSERT INTO ${KEYSPACE}.account_guilds (account_id, guild_id, guild_name, guild_icon, guild_owner, last_synced) VALUES (?,?,?,?,?,?)`, [accountId, realGuildId, guildName, '', false, now]);
                }
                catch (e) {
                    console.warn('[task] account_guilds write failed:', e?.message);
                }
                // 8. Mark task completed
                await db.execute(`UPDATE ${KEYSPACE}.user_tasks SET status = 'completed', updated_at = ? WHERE assigned_to = ? AND task_id = ?`, [now, owner, taskId]);
                // 9. Log
                const ip = getClientIp(req);
                logActivity(db, me.username, 'task_complete', `${guildName || realGuildId} dogrulandi (${accountName}) — ${validatedChannels.channels.length} kanal eklendi`, ip);
            ...
            if (status) {
                await db.execute(`UPDATE ${KEYSPACE}.user_tasks SET status = ?, updated_at = ? WHERE assigned_to = ? AND task_id = ?`, [status, new Date(), owner, taskId]);
            }
            res.json({ ok: true });
        }
        catch (err) {
    ...
    router.delete('/tasks/:taskId', requireAdmin, async (req, res) => {
        try {
            ...
            if (!assignedTo) {
                res.status(400).json({ error: 'assignedTo gerekli' });
                return;
            }
            await db.execute(`DELETE FROM ${KEYSPACE}.user_tasks WHERE assigned_to = ? AND task_id = ?`, [assignedTo, req.params.taskId]);
            res.json({ ok: true });
        }
        catch (err) {
            res.status(500).json({ error: err?.message });
    ...
    router.get('/notifications', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.user_notifications WHERE username = ? LIMIT 50`, [me.username]);
    ...
    router.put('/notifications/:id/read', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            await db.execute(`UPDATE ${KEYSPACE}.user_notifications SET read = true WHERE username = ? AND id = ?`, [me.username, req.params.id]);
            res.json({ ok: true });
        }
        catch (err) {
            res.status(500).json({ error: err?.message });
    ...
    router.get('/my-servers', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            // 1. Get user's tasks
            const taskResult = await db.execute(`SELECT * FROM ${KEYSPACE}.user_tasks WHERE assigned_to = ?`, [me.username]);
            ...
            const poolResult = await db.execute(`SELECT invite_code, guild_id, guild_name, guild_icon, member_count, status, assigned_account_id, assigned_account_name, owner_account_id, owner_account_name FROM ${KEYSPACE}.invite_pool`);
            const poolByGuild = new Map();
            const poolByCode = new Map();
            for (const r of poolResult.rows) {
                if (r['guild_id'])
                    poolByGuild.set(r['guild_id'], r);
                if (r['invite_code'])
                    poolByCode.set(r['invite_code'], r);
            }
            ...
            if (accResult) {
                for (const r of accResult.rows) {
                    accInfo.set(r['account_id'], {
                        username: r['username'] ?? '',
                        avatar: r['avatar'] ?? '',
                        email: r['email'] ?? '',
                        accountPassword: r['account_password'] ?? '',
                        mailPassword: r['mail_password'] ?? '',
                        mailSite: r['mail_site'] ?? '',
                    });
                }
            }
            const servers = [];
            for (const r of guildTasks) {
                ...
                const code = invCode || pool?.['invite_code'] || null;
                servers.push({
                    taskId: r['task_id'],
                    guildId,
                    guildName: r['guild_name'] ?? pool?.['guild_name'] ?? guildId,
                    guildIcon: pool?.['guild_icon'] ?? null,
                    inviteCode: code,
    ...
    router.get('/online', requireAdmin, async (_req, res) => {
        try {
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.user_last_seen`);
            const now = Date.now();
            const users = result.rows.map(r => {
                const lastSeen = r['last_seen'] ? r['last_seen'].getTime() : 0;
                const diffMs = now - lastSeen;
                let status = 'offline';
                if (diffMs < 2 * 60_000)
                    status = 'online';
                else if (diffMs < 10 * 60_000)
                    status = 'away';
                return {
                    username: r['username'],
    ...
    router.get('/activity/:username', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            const target = req.params.username;
            ...
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.user_activity_log WHERE username = ? LIMIT ?`, [target, limit]);
    ...
    router.get('/stats/:username', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            const target = req.params.username;
            if (me.role !== 'admin' && me.username !== target) {
            ...
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.user_tasks WHERE assigned_to = ?`, [target]);
            const tasks = result.rows;
            let pending = 0, inProgress = 0, completed = 0, totalChannels = 0;
            let totalCompletionMs = 0, completedWithTime = 0;
            const dailyCompleted = new Map();
    ...
    router.get('/leaderboard', requireAdmin, async (_req, res) => {
        try {
            const usersResult = await db.execute(`SELECT username, display_name, role FROM ${KEYSPACE}.dashboard_users`);
            const allTasks = await db.execute(`SELECT * FROM ${KEYSPACE}.user_tasks`);
            const byUser = new Map();
            for (const t of allTasks.rows) {
                const u = t['assigned_to'];
                if (!byUser.has(u))
                    byUser.set(u, { completed: 0, pending: 0, inProgress: 0, total: 0, avgMs: 0, count: 0 });
    ...
    router.get('/tasks/:taskId/comments', requireAuth, async (req, res) => {
        try {
            ...
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.task_comments WHERE task_key = ? LIMIT 100`, [taskKey]);
    ...
    router.post('/tasks/:taskId/comments', requireAuth, async (req, res) => {
        try {
            ...
            await db.execute(`INSERT INTO ${KEYSPACE}.task_comments (task_key, comment_id, username, content, created_at) VALUES (?,?,?,?,?)`, [taskKey, commentId, me.username, content.trim(), new Date()]);
            const ip = getClientIp(req);
    ...
    router.get('/sessions', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            const target = req.query.username ?? me.username;
            if (me.role !== 'admin' && me.username !== target) {
                res.status(403).json({ error: 'Forbidden' });
                return;
            }
    ...
    router.delete('/sessions/:sessionId', requireAuth, async (req, res) => {
        try {
            ...
            // Check ownership
            const result = await db.execute(`SELECT username FROM ${KEYSPACE}.user_sessions WHERE session_id = ?`, [sid]);
            if (result.rowLength === 0) {
            ...
            await db.execute(`UPDATE ${KEYSPACE}.user_sessions SET revoked = true WHERE session_id = ?`, [sid]);
            const ip = getClientIp(req);
    ...
    router.post('/force-logout/:username', requireAdmin, async (req, res) => {
        try {
            const target = req.params.username;
            const admin = req.user;
    ...
    router.put('/change-password', requireAuth, async (req, res) => {
        try {
            ...
            if (newPassword.length < 4) {
                res.status(400).json({ error: 'Sifre en az 4 karakter olmali' });
                return;
            }
            // Verify current password
            const result = await db.execute(`SELECT password_hash FROM ${KEYSPACE}.dashboard_users WHERE username = ?`, [me.username]);
            if (result.rowLength === 0) {
                res.status(404).json({ error: 'Kullanici bulunamadi' });
                return;
            }
            ...
            await db.execute(`UPDATE ${KEYSPACE}.dashboard_users SET password_hash = ?, password_changed_at = ? WHERE username = ?`, [hash, new Date(), me.username]);
            const ip = getClientIp(req);
    ...
    router.post('/users/:username/reset-password', requireAdmin, async (req, res) => {
        try {
            ...
            if (!newPassword) {
                res.status(400).json({ error: 'newPassword gerekli' });
                return;
            }
            const hash = await bcryptjs_1.default.hash(newPassword, 12);
            await db.execute(`UPDATE ${KEYSPACE}.dashboard_users SET password_hash = ?, password_changed_at = ? WHERE username = ?`, [hash, new Date(), target]);
            const admin = req.user;
            const ip = getClientIp(req);
            logActivity(db, admin.username, 'password_reset', `${target} sifresi sifirlandi`, ip);
            // Notify user
            const nid = genId();
            db.execute(`INSERT INTO ${KEYSPACE}.user_notifications (id, username, title, message, type, read, created_at) VALUES (?,?,?,?,?,?,?)`, [nid, target, 'Sifre Sifirlandi', 'Sifreniz yonetici tarafindan sifirlandi', 'warning', false, new Date()]).catch(() => { });
            res.json({ ok: true });
        }
        catch (err) {
            res.status(500).json({ error: err?.message });
    ...
    router.get('/page-permissions/:username', requireAuth, async (req, res) => {
        try {
            const me = req.user;
            const target = req.params.username;
            if (me.role !== 'admin' && me.username !== target) {
            ...
            const pages = result.rows.map(r => r['page_id']);
    ...
    router.put('/page-permissions/:username', requireAdmin, async (req, res) => {
        try {
            const target = req.params.username;
            const { pages } = req.body ?? {};
            if (!Array.isArray(pages)) {
                res.status(400).json({ error: 'pages[] gerekli' });
                return;
            }
            ...
            for (const pageId of pages) {
                if (typeof pageId !== 'string')
                    continue;
                await db.execute(`INSERT INTO ${KEYSPACE}.user_page_permissions (username, page_id) VALUES (?,?)`, [target, pageId]).catch(() => { });
            }
            const admin = req.user;
            const ip = getClientIp(req);
    ...
    router.get('/password-policy', requireAdmin, async (_req, res) => {
        try {
            const result = await db.execute(`SELECT * FROM ${KEYSPACE}.password_policy WHERE id = 'global'`);
            if (result.rowLength === 0) {
                res.json({ maxDays: 0, enforce: false, minLength: 4, updatedBy: null, updatedAt: null });
                return;
            }
    ...
    router.put('/password-policy', requireAdmin, async (req, res) => {
        try {
            ...
            const admin = req.user;
            await db.execute(`INSERT INTO ${KEYSPACE}.password_policy (id, max_days, enforce, min_length, updated_by, updated_at) VALUES (?,?,?,?,?,?)`, ['global', Number(maxDays ?? 0), enforce === true, Number(minLength ?? 4), admin.username, new Date()]);
            const ip = getClientIp(req);
    ...
    router.post('/tasks/bulk', requireAdmin, async (req, res) => {
        try {
            ...
            for (const username of usernames) {
                ...
                const taskId = genId();
                await db.execute(`INSERT INTO ${KEYSPACE}.user_tasks (task_id, assigned_to, title, description, status, priority, task_type, deadline, created_by, created_at, updated_at)
           VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [taskId, username, title, description ?? '', 'pending', priority ?? 'medium', 'generic', dl, admin.username, now, now]);
                const notifId = genId();
                await db.execute(`INSERT INTO ${KEYSPACE}.user_notifications (id, username, title, message, type, read, created_at) VALUES (?,?,?,?,?,?,?)`, [notifId, username, 'Yeni Gorev', `"${title}" gorevi size atandi`, 'task', false, now]).catch(() => { });
