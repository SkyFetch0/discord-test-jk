...
exports.systemStatsRouter = systemStatsRouter;
...
function systemStatsRouter(scylla, ch, brokers) {
      ...
      `;
            ...
            if (scyllaEst.status === 'fulfilled' && scyllaEst.value.rows) {
                for (const row of scyllaEst.value.rows) {
                    ...
                    if (!tableAgg[tn])
                        tableAgg[tn] = { bytes: 0, partitions: 0 };
            ...
            for (const tn of scyllaMsgNames)
                scyllaMsgBytes += tableAgg[tn]?.bytes ?? 0;
...
// ── Kafka helper ─────────────────────────────────────────────
async function fetchKafkaInfo(brokers) {
        ...
        for (const topic of userTopics) {
            try {
                ...
                for (const p of offsets) {
                    const high = parseInt(String(p.high ?? p.offset ?? '0'), 10);
                    const low = parseInt(String(p.low ?? '0'), 10);
                    totalProduced += high;
