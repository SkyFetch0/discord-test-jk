...
const _msgDedup = new Map<string, 1>();

function seenMessageId(id: string): boolean {
  if (_msgDedup.has(id)) return true;
  if (_msgDedup.size >= DEDUP_CACHE_MAX) {
    // Evict oldest entry (Map preserves insertion order)
    const first = _msgDedup.keys().next().value;
    if (first !== undefined) _msgDedup.delete(first);
  }
  _msgDedup.set(id, 1);
  return false;
}
...
function identityCacheKey(authorId: string, field: string, guildId?: string): string {
  return guildId && guildId !== '0' ? `${authorId}:${field}:${guildId}` : `${authorId}:${field}`;
}

function identityChanged(authorId: string, field: string, value: string, guildId?: string): boolean {
  const key = identityCacheKey(authorId, field, guildId);
  const prev = _identityCache.get(key);
  if (prev === value) return false; // no change
  ...
  if (_identityCache.size >= IDENTITY_CACHE_MAX && !_identityCache.has(key)) {
    ...
    if (firstKey !== undefined) _identityCache.delete(firstKey);
  }
...
export async function writeMessages(
  ...
  const valid = messages.filter(
    m => m.messageId && m.channelId && m.guildId && m.authorId && m.ts
      && !seenMessageId(m.messageId), // FIX: skip already-seen message_ids
  );
  ...
  // Build identity log rows — LRU cache reduces write volume, ReplacingMergeTree handles correctness.
  // observed_ts = NOW (ingestion time), NOT message creation time.
  // source_msg_ts = message time (informational only).
  type IdRow = { author_id: string; field: string; value: string; guild_id: string; observed_ts: string; source_msg_ts: string };
  const identityRows: IdRow[] = [];
  const seenAuthors = new Set<string>();

  for (const m of valid) {
    if (seenAuthors.has(m.authorId)) continue;
