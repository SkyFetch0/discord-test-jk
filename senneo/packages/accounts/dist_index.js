...
async function getServerIp() {
    ...
    _serverIpPromise = new Promise(resolve => {
        const req = https_1.default.request({ hostname: 'api.ipify.org', path: '/', method: 'GET', headers: { 'User-Agent': 'Mozilla/5.0' } }, (res) => {
            let d = '';
            res.on('data', (c) => d += c.toString());
            res.on('end', () => { _serverIp = d.trim(); resolve(_serverIp); });
        });
...
async function verifyProxyIp(gIdx, proxyUrl, proxyLabel) {
    ...
    const proxyIp = await new Promise(resolve => {
        try {
            ...
            req.on('error', (e) => { console.warn(`[proxy-verify] idx=${gIdx} ${proxyLabel} IP check hatası: ${e.message}`); resolve(null); });
            req.setTimeout(8000, () => { req.destroy(); resolve(null); });
            req.end();
        }
        catch (e) {
            console.warn(`[proxy-verify] idx=${gIdx} ${proxyLabel} parse hatası: ${e.message}`);
            resolve(null);
        }
    });
        ...
        const req = https_1.default.request({ hostname: 'discord.com', path: '/api/v10/users/@me', method: 'GET',
            ...
            res.on('end', () => {
                try {
                    ...
                    // 401 = gerçek token hatası
                    if (statusCode === 401) return resolve({ _tokenInvalid: true, _statusCode: statusCode });
                    // 4xx/5xx diğerleri = proxy/ağ sorunu
                    if (statusCode >= 400) return resolve({ _networkError: true, _statusCode: statusCode });
                    resolve(parsed);
                } catch {
                    resolve({ _networkError: true, _statusCode: statusCode });
                }
            });
        });
...
async function main() {
    ...
    for (let i = 0; i < accountPairs.length; i += LOGIN_CONCURRENCY) {
        ...
        await Promise.all(chunk.map(async ([gIdx, acc]) => {
            ...
            console.log(`[login-plan] idx=${gIdx} token=${tokenHint2} proxyEnabled=${(0, proxy_1.isProxyPoolEnabled)()} strictMode=${(0, proxy_1.isProxyStrictMode)()} proxyAssigned=${!!proxyAssignment?.proxy} proxyLabel=${proxyAssignment?.proxy?.label ?? 'none'}`);
            let bundle;
            try {
                if ((0, proxy_1.isProxyPoolEnabled)()) {
                    if (!proxyAssignment?.proxy) {
                        const noProxyMsg = 'Proxy sistemi aktif ama hesaba atanmış proxy yok';
                        console.warn(`[login-plan] idx=${gIdx} — proxy yok! strictMode=${(0, proxy_1.isProxyStrictMode)()}`);
                        ...
                        if (!verify.ok && (0, proxy_1.isProxyStrictMode)()) {
                ...
                // Pre-flight: token ve proxy/ağ erişimini doğrula
                ...
                if (preflightMe?._networkError) {
                    ...
                    return; // network hatası → skip, fatal sayma
                }
                if (preflightMe?._tokenInvalid || !preflightMe?.id) {
                    const preflightErr = new Error('An invalid token was provided.');
                ...
                const errCode = err?.code ?? err?.[Symbol.for('code')] ?? 'no_code';
    ...
    async function finalizeScrapeExit(target, accId, runToken, result) {
        ...
        if (result.kind === 'completed') {
            ...
            // Emit yield event
            (0, scrape_event_log_1.emit)('info', `${target.channelId} re-queued`, {
                accountId: accId,
            ...
            if (abort.signal.aborted) {
                ...
                try {
                    result = await (0, scraper_1.scrapeChannel)(client, target.guildId, target.channelId, async (batch) => { if (!abort.signal.aborted)
                        ...
                        console.log(`[accounts] ${accName} | ${target.channelId} | ${total.toLocaleString()} mesaj`); }, abort.signal, accId, throttleHooks, client.token, // RAW HTTP: Pass token for 5x faster message fetching
                    accountAgents.get(accId)?.proxyUrl);
                }
                catch (err) {
                    if (!abort.signal.aborted)
