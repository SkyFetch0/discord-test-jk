...
import { AccountConfig, sleep } from '@senneo/shared';
...
function fetchGuilds(token: string, agent?: Agent): Promise<DiscordGuildPartial[]> {
...
async function syncOneAccount(
  ...
  const existing = await db.execute(
    `SELECT guild_id FROM ${KEYSPACE}.account_guilds WHERE account_id = ?`,
    [account.accountId],
  );
  const freshIds = new Set(guilds.map((g) => g.id));
  const toRemove = existing.rows
    .map((r) => r['guild_id'] as string)
    .filter((gid) => !freshIds.has(gid));

  if (toRemove.length > 0) {
    await Promise.all(
      toRemove.flatMap((gid) => [
        db.execute(
          `DELETE FROM ${KEYSPACE}.account_guilds WHERE account_id = ? AND guild_id = ?`,
          [account.accountId, gid],
        ),
        db.execute(
          `DELETE FROM ${KEYSPACE}.guild_accounts WHERE guild_id = ? AND account_id = ?`,
          [gid, account.accountId],
        ),
      ]),
    );
...
async function runFullSync(
  ...
  await db.execute(
    `INSERT INTO ${KEYSPACE}.guild_sync_status (id, syncing, total_accounts, synced_accounts, total_guilds) VALUES ('current',true,?,0,0)`,
    [accounts.length],
  ).catch(() => {});
  ...
  for (const account of accounts) {
    ...
    // Update progress
    await db.execute(
      `UPDATE ${KEYSPACE}.guild_sync_status SET synced_accounts = ?, total_guilds = ? WHERE id = 'current'`,
      [_syncedAccounts, _totalGuilds],
    ).catch(() => {});

    // Rate limit delay between accounts
    if (DELAY_PER_ACCOUNT_MS > 0) await sleep(DELAY_PER_ACCOUNT_MS);
  }
  ...
  const uniqueResult = await db.execute(
    `SELECT guild_id FROM ${KEYSPACE}.guild_accounts`,
  ).catch(() => null);
  const uniqueGuilds = uniqueResult ? new Set(uniqueResult.rows.map((r: any) => r['guild_id'])).size : _totalGuilds;

  await db.execute(
    `INSERT INTO ${KEYSPACE}.guild_sync_status (id, last_sync_at, syncing, total_accounts, synced_accounts, total_guilds) VALUES ('current',?,false,?,?,?)`,
    [new Date(), accounts.length, _syncedAccounts, uniqueGuilds],
  ).catch(() => {});
  ...
  await postSyncVerify(db).catch((err) =>
    console.error('[guild-sync] Post-sync verify error:', err),
  );
}
...
async function postSyncVerify(db: CassandraClient): Promise<void> {
  const pool = await db.execute(
    `SELECT invite_code, guild_id, status, owner_account_id FROM ${KEYSPACE}.invite_pool`,
  );
  if (pool.rowLength === 0) return;
  ...
  const accInfoResult = await db.execute(
    `SELECT account_id, username FROM ${KEYSPACE}.account_info`,
  ).catch(() => null);
  const accLabels = new Map<string, string>();
  if (accInfoResult) {
  ...
  for (const row of pool.rows) {
    ...
    if (status === 'to_join' && isKnown) {
      ...
      await db.execute(
        ...
        [ownerId, ownerLabel, now, code],
      ).catch(() => {});
      joined++;
    } else if (status === 'already_in' && !isKnown) {
      await db.execute(
        `UPDATE ${KEYSPACE}.invite_pool SET status = 'to_join', owner_account_id = ?, owner_account_name = ?, checked_at = ? WHERE invite_code = ?`,
        [null, null, now, code],
      ).catch(() => {});
      left++;
    } else if (status === 'already_in' && isKnown) {
      // Owner left but another account is still in → transfer ownership
      const currentOwner = (row['owner_account_id'] as string) ?? '';
      if (currentOwner && !owners.includes(currentOwner)) {
        ...
        await db.execute(
          ...
          [newOwnerId, newOwnerLabel, now, code],
        ).catch(() => {});
        transferred++;
      }
    }
...
// ── Public API ─────────────────────────────────────────────────────────────
let _db: CassandraClient | null = null;
let _accounts: GuildSyncAccount[] = [];

export function startGuildSync(
  db: CassandraClient,
  accounts: GuildSyncAccount[],
): void {
  _db = db;
  ...
  setTimeout(() => {
    runFullSync(db, accounts).catch((err) =>
      console.error('[guild-sync] Initial sync error:', err),
    );
  }, 10_000);

  // Periodic sync
  _timer = setInterval(() => {
    runFullSync(_db!, _accounts).catch((err) =>
      console.error('[guild-sync] Periodic sync error:', err),
    );
  }, SYNC_INTERVAL_MS);
...
export function updateGuildSyncAccounts(accounts: GuildSyncAccount[]): void {
  _accounts = accounts;
}

export async function triggerGuildSync(): Promise<void> {
  if (!_db || !_accounts.length) throw new Error('Guild sync not initialized');
...
export function getGuildSyncState(): { syncing: boolean; lastSyncAt: string | null; syncedAccounts: number; totalGuilds: number } {
